...
Default value is ``false`` (except if ``changes_format`` is ``classic``).
...
Default value is ``release_summary``.
...
Default value is ``Release Summary``.
...
Default value is the titlecase of the collection's namespace and name.
...
Default value is ``false`` for existing configurations, and ``true`` for new configurations.
...
Default value is ``'(?P<pre_release>\.\d+(?:[ab]|rc)+\d*)$'``.
