...
"""
    ...
    assert (
        collection_changelog.run_tool("generate", ["-v", "--refresh"]) == C.RC_SUCCESS
    )
...
def test_changelog_release_simple(  # pylint: disable=redefined-outer-name
    ...
    collection_changelog.set_plugin_cache(
        "1.0.0",
        {
            "module": {
                "test": {
    ...
    assert collection_changelog.run_tool("lint", ["-vv"]) == C.RC_INVALID_FRAGMENT
    ...
    # Lint fragments
    assert collection_changelog.run_tool("lint", ["-vv"]) == C.RC_SUCCESS
...
"""
    ...
    assert collection_changelog.run_tool("generate", ["-v"]) == C.RC_SUCCESS
    assert collection_changelog.diff().unchanged
...
"""
    ...
    collection_changelog.add_fragment_line("bugfix.yml", "bugfixes", ["A bugfix."])
...
"""
    ...
    collection_changelog.add_fragment_generic(
        ...
        {
            "add plugin.module": [
                {"name": "meh", "description": "A meh module", "namespace": "foo"},
            ],
        },
    )
        ...
        "acme.test.cloud.sky.old_module": {
            "doc": {
                ...
                "filename": os.path.join(
                    "plugins", "modules", "cloud", "sky", "old_module.py"
                ),
                "name": "old_module",
                "options": {},
